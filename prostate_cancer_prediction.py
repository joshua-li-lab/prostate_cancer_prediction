    ...
    for n in Ns:
        for cutoff in cutoffs:
            ...
            for key in cur_patients:
                ...
                if len(grp) < n: # Now only need at least n measurements for features (predict cancer from first n)
                    continue
                psa = grp['LIS Result: Numeric Result'].values
                times_str = grp['LIS Reference Datetime'].values
                # Parse each datetime individually to avoid format inference warning
                try:
            ...
            num_cancer_in_N_below = sum(y_list)
            print(f"Patients with first {n} <= {cutoff}: {num_patients_N_below} (cancer: {num_cancer_in_N_below})")
            if num_patients_N_below > 0:
                cancer_rate = num_cancer_in_N_below / num_patients_N_below
            else:
                cancer_rate = 0.0
               
            print(f"Patients with first {n} <= {cutoff}: {num_patients_N_below} (cancer: {num_cancer_in_N_below})")
            print(f"► Cancer rate in this early low-PSA cohort: {cancer_rate:.1%} ({num_cancer_in_N_below}/{num_patients_N_below})")
           
            if len(np.unique(y)) < 2:
                print("Only one class present. Skipping.")
                continue
            # Balance the dataset by downsampling the majority class (non-cancer) to match cancer positives
            pos_idx = np.where(y == 1)[0] # Assuming label 1 is cancer
            ...
            if len(pos_idx) > 0 and len(neg_idx) > len(pos_idx):
                ...
                balanced_num_cancer = sum(y_bal)
                print(f"Balanced dataset size: {balanced_num_patients} (cancer: {balanced_num_cancer})")
            else:
                X_bal = X
                y_bal = y
                patient_bal = np.array(patient_list)
                balanced_num_patients = num_patients_N_below
                balanced_num_cancer = num_cancer_in_N_below
            ...
            for fold, (train_full_idx, test_idx) in enumerate(skf.split(X_bal, y_bal)):
                ...
                patients_train_full = patient_bal[train_full_idx]
                patients_test = patient_bal[test_idx]
                ...
                print(f"Number of rows in training data: {len(patients_train) * n}")
                ...
                print(f"Number of rows in validation data: {len(patients_val) * n}")
                ...
                plt.xticks(tick_marks, ['No Cancer', 'Cancer'], rotation=45)
            ...
            results.append({
                ...
                'num_cancer_in_N_below': num_cancer_in_N_below,
                'balanced_num_patients': balanced_num_patients,
                'balanced_num_cancer': balanced_num_cancer,
                'acc': avg_acc,
                'prec': avg_prec,
                'rec': avg_rec,
                'f1': avg_f1,
